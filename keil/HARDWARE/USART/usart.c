...
{
	usart1_buf[usart1_buf_len] =USART_ReceiveData(USART1);	//读取接收到的数据
	usart1_buf_len++;
}
//void USART1_IRQHandler(void)                	//串口1中断服务程序
//{
...
{
	usart2_buf[usart2_buf_len] =USART_ReceiveData(USART2);	//读取接收到的数据
	usart2_buf_len++;
}
...
{
	usart3_buf[usart3_buf_len] =USART_ReceiveData(USART3);	//读取接收到的数据
	usart3_buf_len++;
}
